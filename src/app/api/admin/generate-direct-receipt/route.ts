...
export async function POST(request: NextRequest) {
  try {
    ...
    doc.text(`#${receiptNumber}`, 0, 25, { align: 'right' });
    ...
    const response = new NextResponse(pdfBuffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="akrix-receipt-${receiptNumber}.pdf"`,
    ...
    console.error('Error generating direct receipt:', error);
    return NextResponse.json(
      { success: false, message: 'Failed to generate receipt', error: error.message },
      { status: 500 }
    );
  }
}